// src/lib/font/font8x8_basic.c
#include <ui/font8x8_basic.h>

/*
 * Çok basit bir 8x8 font tablosu.
 * Tam değil ama test için yeterli: rakamlar, bazı harfler vs.
 * Tanımlanmayan karakterler boş (hepsi 0) olacak.
 */

const uint8_t font8x8_basic[256][8] = {
    [' '] = { 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },

    ['0'] = { 0x3C,0x66,0x6E,0x76,0x66,0x66,0x3C,0x00 },
    ['1'] = { 0x18,0x38,0x18,0x18,0x18,0x18,0x3C,0x00 },
    ['2'] = { 0x3C,0x66,0x06,0x0C,0x30,0x60,0x7E,0x00 },
    ['3'] = { 0x3C,0x66,0x06,0x1C,0x06,0x66,0x3C,0x00 },
    ['^'] = { 0x10,0x38,0x6C,0x00,0x00,0x00,0x00,0x00 }, // ^
    ['4'] = { 0x0C,0x1C,0x3C,0x6C,0x7E,0x0C,0x0C,0x00 },
    ['5'] = { 0x7E,0x60,0x7C,0x06,0x06,0x66,0x3C,0x00 },
    ['6'] = { 0x1C,0x30,0x60,0x7C,0x66,0x66,0x3C,0x00 },
    ['7'] = { 0x7E,0x06,0x0C,0x18,0x30,0x30,0x30,0x00 },
    ['8'] = { 0x3C,0x66,0x66,0x3C,0x66,0x66,0x3C,0x00 },
    ['9'] = { 0x3C,0x66,0x66,0x3E,0x06,0x0C,0x38,0x00 },

    ['A'] = { 0x18,0x3C,0x66,0x66,0x7E,0x66,0x66,0x00 },
    ['B'] = { 0x7C,0x66,0x66,0x7C,0x66,0x66,0x7C,0x00 },
    ['C'] = { 0x3C,0x66,0x60,0x60,0x60,0x66,0x3C,0x00 },    
    [24] = { 0x3C, 0x66, 0x60, 0x60, 0x60, 0x66, 0x3C, 0x18 }, // Ç
    ['D'] = { 0x78,0x6C,0x66,0x66,0x66,0x6C,0x78,0x00 },
    ['E'] = { 0x7E,0x60,0x60,0x7C,0x60,0x60,0x7E,0x00 },
    ['F'] = { 0x7E,0x60,0x60,0x7C,0x60,0x60,0x60,0x00 },
    ['G'] = { 0x3C,0x66,0x60,0x6E,0x66,0x66,0x3C,0x00 },    
    [20] = { 0x1C, 0x00, 0x3E, 0x60, 0x6E, 0x66, 0x3E, 0x00 }, // Ğ
    ['H'] = { 0x66,0x66,0x66,0x7E,0x66,0x66,0x66,0x00 },
    ['I'] = { 0x3C,0x18,0x18,0x18,0x18,0x18,0x3C,0x00 },
    [22] = { 0x18, 0x00, 0x3C, 0x18, 0x18, 0x18, 0x18, 0x3C }, // İ (Nokta 1. satırda)
    ['J'] = { 0x1E,0x0C,0x0C,0x0C,0x0C,0x6C,0x38,0x00 },
    ['K'] = { 0x66,0x6C,0x78,0x70,0x78,0x6C,0x66,0x00 },
    ['L'] = { 0x60,0x60,0x60,0x60,0x60,0x60,0x7E,0x00 },
    ['M'] = { 0x63,0x77,0x7F,0x6B,0x63,0x63,0x63,0x00 },
    ['N'] = { 0x66,0x76,0x7E,0x7E,0x6E,0x66,0x66,0x00 },
    ['O'] = { 0x3C,0x66,0x66,0x66,0x66,0x66,0x3C,0x00 },    
    [23] = { 0x24, 0x00, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00 }, // Ö
    ['P'] = { 0x7C,0x66,0x66,0x7C,0x60,0x60,0x60,0x00 },
    ['Q'] = { 0x3C,0x66,0x66,0x66,0x6E,0x6C,0x36,0x00 },
    ['R'] = { 0x7C,0x66,0x66,0x7C,0x78,0x6C,0x66,0x00 },
    ['S'] = { 0x3C,0x66,0x30,0x18,0x0C,0x66,0x3C,0x00 },    
    [21] = { 0x3E, 0x60, 0x60, 0x3C, 0x06, 0x06, 0x3E, 0x18 }, // Ş
    ['T'] = { 0x7E,0x5A,0x18,0x18,0x18,0x18,0x3C,0x00 },
    ['U'] = { 0x66,0x66,0x66,0x66,0x66,0x66,0x3C,0x00 },    
    [25] = { 0x24, 0x00, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00 }, // Ü
    ['V'] = { 0x66,0x66,0x66,0x66,0x66,0x3C,0x18,0x00 },
    ['W'] = { 0x63,0x63,0x63,0x6B,0x7F,0x77,0x63,0x00 },
    ['X'] = { 0x66,0x66,0x3C,0x18,0x3C,0x66,0x66,0x00 },
    ['Y'] = { 0x66,0x66,0x3C,0x18,0x18,0x18,0x3C,0x00 },
    ['Z'] = { 0x7E,0x06,0x0C,0x18,0x30,0x60,0x7E,0x00 },

    // 8x8 lowercase a-z (readable, simple)
    ['a'] = { 0x00,0x00,0x3C,0x06,0x3E,0x66,0x3E,0x00 }, // a
    ['b'] = { 0x60,0x60,0x7C,0x66,0x66,0x66,0x7C,0x00 }, // b
    ['c'] = { 0x3C, 0x66, 0x60, 0x66, 0x3C, 0x00, 0x00, 0x00 }, // c
    [18] =  { 0x3C, 0x66, 0x60, 0x66, 0x3C, 0x00, 0x18, 0x00 }, // ç (Kuyruk 7-8'de)
    ['d'] = { 0x06,0x06,0x3E,0x66,0x66,0x66,0x3E,0x00 }, // d
    ['e'] = { 0x00,0x00,0x3C,0x66,0x7E,0x60,0x3C,0x00 }, // e
    ['f'] = { 0x1C,0x30,0x7C,0x30,0x30,0x30,0x30,0x00 }, // f
    ['g'] = { 0x00,0x00,0x3E,0x66,0x66,0x3E,0x06,0x3C }, // g
    [14] = { 0x1C, 0x00, 0x3E, 0x66, 0x66, 0x3E, 0x06, 0x3C }, // ğ (Şapka 1. satırda, gövde 3'te)
    ['h'] = { 0x60,0x60,0x7C,0x66,0x66,0x66,0x66,0x00 }, // h
    [16] =  { 0x00, 0x00, 0x38, 0x18, 0x18, 0x18, 0x3C, 0x00 }, // ı
    ['i'] = { 0x18,0x00,0x38,0x18,0x18,0x18,0x3C,0x00 }, // i
    ['j'] = { 0x0C,0x00,0x1C,0x0C,0x0C,0x6C,0x6C,0x38 }, // j
    ['k'] = { 0x60,0x60,0x66,0x6C,0x78,0x6C,0x66,0x00 }, // k
    ['l'] = { 0x38,0x18,0x18,0x18,0x18,0x18,0x3C,0x00 }, // l
    ['m'] = { 0x00,0x00,0x6C,0x7E,0x7E,0x6B,0x63,0x00 }, // m
    ['n'] = { 0x00,0x00,0x7C,0x66,0x66,0x66,0x66,0x00 }, // n
    ['o'] = { 0x00,0x00,0x3C,0x66,0x66,0x66,0x3C,0x00 }, // o
    [17] = { 0x24, 0x00, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00 }, // ö
    ['p'] = { 0x00,0x00,0x7C,0x66,0x66,0x7C,0x60,0x60 }, // p
    ['q'] = { 0x00,0x00,0x3E,0x66,0x66,0x3E,0x06,0x06 }, // r
    ['r'] = { 0x00,0x00,0x6E,0x70,0x60,0x60,0x60,0x00 }, // r
    ['s'] = { 0x00,0x00,0x3E,0x60,0x3C,0x06,0x7C,0x00 }, // s
    [15] = {                                             // ş

    },
    ['t'] = { 0x30,0x30,0x7C,0x30,0x30,0x30,0x1C,0x00 }, // t
    ['u'] = { 0x00,0x00,0x66,0x66,0x66,0x66,0x3E,0x00 }, // u
    [19]  = {                                            // ü

    },
    ['v'] = { 0x00,0x00,0x66,0x66,0x66,0x3C,0x18,0x00 }, // v
    ['w'] = { 0x00,0x00,0x63,0x63,0x6B,0x7F,0x36,0x00 }, // w
    ['x'] = { 0x00,0x00,0x66,0x3C,0x18,0x3C,0x66,0x00 }, // z
    ['y'] = { 0x00,0x00,0x66,0x66,0x66,0x3E,0x06,0x3C }, // y
    ['z'] = { 0x00,0x00,0x7E,0x0C,0x18,0x30,0x7E,0x00 }, // z

    // ñ  (U+00F1)  -> tek-byte latin1/cp1252: 0xF1
    [0xF1] = { 0x00,0x00,0x36,0x00,0x7C,0x66,0x66,0x66 }, // ñ

    ['.'] = { 0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x00 },
    [','] = { 0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x10 },
    [':'] = { 0x00,0x18,0x18,0x00,0x00,0x18,0x18,0x00 },
    [';'] = { 0x00,0x18,0x18,0x00,0x00,0x18,0x18,0x10 },
    ['-'] = { 0x00,0x00,0x00,0x7E,0x00,0x00,0x00,0x00 },
    ['_'] = { 0x00,0x00,0x00,0x00,0x00,0x00,0x7E,0x00 },
    ['/'] = { 0x06,0x0C,0x18,0x30,0x60,0x40,0x00,0x00 },
    ['\\']= { 0x60,0x30,0x18,0x0C,0x06,0x02,0x00,0x00 },
    ['!'] = { 0x18,0x18,0x18,0x18,0x18,0x00,0x18,0x00 },
    ['?'] = { 0x3C,0x66,0x06,0x0C,0x18,0x00,0x18,0x00 },

    // Apostrophe '
    ['\''] = { 0x18,0x18,0x10,0x00,0x00,0x00,0x00,0x00 },

    // Backtick `
    ['`']  = { 0x18,0x18,0x08,0x00,0x00,0x00,0x00,0x00 },

    // Greater-than >
    ['>']  = { 0x00,0x60,0x30,0x18,0x30,0x60,0x00,0x00 },

    // Less-than <
    ['<']  = { 0x00,0x06,0x0C,0x18,0x0C,0x06,0x00,0x00 },

    // Equals =
    ['=']  = { 0x00,0x00,0x7E,0x00,0x7E,0x00,0x00,0x00 },

    // Pipe |
    ['|']  = { 0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x00 },

    // Double quote "
    ['"']  = { 0x36,0x36,0x24,0x00,0x00,0x00,0x00,0x00 },
};
