...
static fat_info_t g_fat_info;
static int g_fat_ready = 0;

static int fat_ensure_mounted(void)
{
    if (g_fat_ready) return 1;

    uint8_t sector[512];
    if (!block_has_root()) return 0;
    if (!block_read(0, 1, sector)) return 0;
    if (!fat_probe_from_sector0(sector, &g_fat_info)) return 0;
...
int fat_mount_probe(void)
{
    g_fat_ready = 0;
    return fat_ensure_mounted();
}
...
static int fat_list_root_entries(const fat_info_t* info)
{
    if (!info) return 0;
    if (info->type != FAT_TYPE_16 && info->type != FAT_TYPE_12) {
...
{
    if (!fat_ensure_mounted()) {
        printk("[FAT] invalid FAT boot sector\n");
        return 0;
    }

    return fat_list_root_entries(&g_fat_info);
}

static void fat_commands_print_entry(const fat_dirent_t* de)
{
    char namebuf[20];
    fat_build_short_name(de, namebuf, sizeof(namebuf));
    ...
    if (de->attr & FAT_ATTR_DIRECTORY) {
        commands_puts("[DIR]  ");
        commands_puts(namebuf);
        commands_puts("\n");
    } else {
        commands_printf("%u byte  %s\n", de->file_size, namebuf);
    }
}

static int fat_list_root_entries_cmd(const fat_info_t* info)
{
    if (!info) return 0;
    ...
    for (uint32_t s = 0; s < info->root_dir_sectors; s++) {
        if (!block_read(root_dir_start + s, 1, sec)) return 0;
...
static int fat_list_dir_cluster_cmd(const fat_info_t* info, uint32_t dir_cluster)
{
    if (!info) return 0;
    if (info->type != FAT_TYPE_16) return 0;
    if (dir_cluster < 2) return 0;
...
int fat_list_path_cmd(const char* path)
{
    if (!fat_ensure_mounted()) return 0;

    if (!path || strcmp(path, "") == 0 || strcmp(path, "/") == 0) {
        return fat_list_root_entries_cmd(&g_fat_info);
    }

    fat_dirent_t de;
    int is_root = 0;

    if (!fat_resolve_path(&g_fat_info, path, &de, &is_root)) return 0;
    if (is_root) return fat_list_root_entries_cmd(&g_fat_info);
    if (!(de.attr & FAT_ATTR_DIRECTORY)) return 0;
    ...
    return fat_list_dir_cluster_cmd(&g_fat_info, cluster);
}
...
{
    if (!fat_ensure_mounted()) return 0;
    if (g_fat_info.type != FAT_TYPE_16 && g_fat_info.type != FAT_TYPE_12) return 0;
    ...
    if (!fat_resolve_path(&g_fat_info, path, &de, &is_root)) {
        return 0;
    }
    ...
    return fat_list_dir_cluster(&g_fat_info, cluster);
}
...
int fat_read_file_path(const char* path, uint8_t* out, uint32_t out_cap, uint32_t* out_size)
{
    ...
    if (!fat_ensure_mounted()) return 0;
    if (g_fat_info.type != FAT_TYPE_12 && g_fat_info.type != FAT_TYPE_16) return 0;
    ...
    while (cluster >= 2 && copied < file_size) {
        uint32_t lba = fat_cluster_to_lba(&g_fat_info, cluster);
        if (lba == 0) return 0;

        for (uint32_t s = 0; s < g_fat_info.sectors_per_cluster && copied < file_size; s++) {
            if (!block_read(lba + s, 1, sec)) return 0;
        ...
        if (g_fat_info.type == FAT_TYPE_16) {
            uint32_t next = fat16_next_cluster(&g_fat_info, cluster);
            if (next == 0xFFFFFFFF) break;
            if (next < 2) return 0;
            cluster = next;
        } else {
