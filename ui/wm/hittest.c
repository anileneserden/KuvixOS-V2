...
wm_hittest_t ui_chrome_hittest(const ui_window_t* win, int mx, int my) {
    ...
    if (mx >= L.btn_close_x && mx < L.btn_close_x + L.btn_w &&
        my >= L.btn_y       && my < L.btn_y       + L.btn_h) return HT_BTN_CLOSE;

    if (mx >= L.btn_max_x && mx < L.btn_max_x + L.btn_w &&
        my >= L.btn_y     && my < L.btn_y     + L.btn_h) return HT_BTN_MAX;

    if (mx >= L.btn_min_x && mx < L.btn_min_x + L.btn_w &&
        my >= L.btn_y     && my < L.btn_y     + L.btn_h) return HT_BTN_MIN;
