...
bool perform_installation(ata_disk_t* dest_disk) {
    if (!dest_disk) return false;
...
void ui_installer_retro_run(void) {
    int stage = 0;
    int selected_disk = 0;
    bool done = false;
    ...
    while (!done) {
        ...
        if (stage == 0) {
            ...
            gfx_fill_rect(wx + 40, wy + 200, 500, 32, 0x0000AA);
            ...
            gfx_fill_rect(wx + 40, wy + 250, 500, 32, 0x0000AA);
        ...
        if (sc == 0x48) { // YUKARI
            if (stage == 1 && selected_disk > 0) {
                selected_disk--;
            }
        }

        if (sc == 0x50) { // AŞAĞI
            if (stage == 1 && selected_disk < ata_pio_get_disk_count() - 1) {
                selected_disk++;
            }
        }
